    ...
    <div className="flex min-h-screen bg-[#E6F0F5]">
      ...
      `}>
        <div className="px-6 py-6 flex items-center justify-between">
          <Link to="/dashboard" className="flex items-center gap-3" style={{ height: '46px' }}>
            <img 
              src={mainLogo} 
              alt="Sofi Core Logo" 
              className="w-10 h-10 object-contain"
            />
            <span className="font-bold text-xl leading-none" style={{ color: '#C59E54', marginTop: '4px' }}>Soul Gate</span>
          </Link>
        ...
        <nav className="flex-1 px-3 pt-2 overflow-y-auto">
          <ul className="space-y-1">
            {menuItems.filter(item => item.path !== '/subscription').map((item) => {
                ...
                <li key={item.path} className="relative">
                  ...
                  <Link
                    ...
                    style={{
                      ...
                      letterSpacing: '0.3px',
                      gap: '12px',
                      background: isActive ? 'linear-gradient(to right, #275681, #4F94D4)' : 'transparent'
                    }}
                  >
